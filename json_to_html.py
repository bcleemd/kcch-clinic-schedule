...
Changes:
    - refactor: output HTML, CSS, and JS files to project root only (removed web_home)
"""
...
# Predefined high-visibility colors for doctors
DOCTOR_COLORS = [
    "#d97706", # Amber
    "#059669", # Emerald
    "#7c3aed", # Violet
...
def get_doctor_color(name):
    global color_index
    # Extract base name by removing anything in parentheses (e.g., "김진(초음파)" -> "김진")
    base_name = re.sub(r'\(.*?\)', '', name).strip()
    
    if base_name not in doctor_color_map:
        doctor_color_map[base_name] = DOCTOR_COLORS[color_index % len(DOCTOR_COLORS)]
        color_index += 1
    return doctor_color_map[base_name]
...
def format_doctors_html(doctors):
    ...
    formatted = []
    for doc in doctors:
        color = get_doctor_color(doc)
        # Check for parentheses to wrap position for mobile line break
        if '(' in doc and ')' in doc:
            name_part = doc.split('(', 1)[0]
            pos_part = '(' + doc.split('(', 1)[1]
            formatted.append(f'<span style="color: {color}; font-weight: 600;">{name_part}<span class="mobile-block">{pos_part}</span></span>')
        else:
            formatted.append(f'<span style="color: {color}; font-weight: 600;">{doc}</span>')
    
    return "<br>".join(formatted)

def main():
    if len(sys.argv) < 2:
        print("Usage: uv run json_to_html.py <input_json_file>")
        return
...
"""
    ...
    for dept in departments:
        ...
        table_body_html += f'    <td class="time-cell">오<span class="mobile-block"></span>전</td>\n'
        for day in days:
        ...
        table_body_html += f'    <td class="time-cell">오<span class="mobile-block"></span>후</td>\n'
        for day in days:
...
"""
    ...
    print(f"Successfully generated (project root):")
    print(f"  - {html_path}")
    print(f"  - {css_path}")
    print(f"  - {js_path}")
